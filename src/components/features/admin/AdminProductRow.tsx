...
function AdminProductRow({ product }: Props) {
  ...
  return (
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
      ...
      {/* Category */}
      <td className="px-6 py-1">
        <div className="text-xs">{product.category?.toString()}</div>
      </td>
      ...
      <td className="px-6">
        <div className="flex gap-1 max-w-[20rem] overflow-x-scroll">
          {product.priceHistory?.map((datedPrice, i) => {
            return (
              <DatedPriceTag
                datedPrice={datedPrice}
                key={datedPrice?.date?.toString() || i}
              />
            );
          })}
        </div>
